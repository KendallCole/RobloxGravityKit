...
function GravityControllerClass.new(player)
	...
	self.Player = player
	self.Character = player.Character or player.CharaterAdded:Wait()
	self.Humanoid = player.Character:WaitForChild("Humanoid")
	
	self.HRP = nil 
	while self.HRP == nil do 
...
local function onJumpRequest(self)
	...
	if not self.StateTracker.Jumped and grounded then
...
local function onDoubleJumpRequest(self)
	local grounded = self._collider:IsGrounded(true)
	--if grounded, we will always need our jump to be gone
	if grounded then
...
local function onGravityStep(self, dt)
	...
	if false and self._camera.CameraModule:IsCamRelative() then
	...
	if not self._collider:IsGrounded()then
		self._collider:SetAirFriction()
	else
		self._collider:SetGroundFriction()
	end
	local scale = self.Character:GetScale()
	
	self._collider:Update(walkForce * scale + gForce, charRotation)
end
...
function init(self)
	...
	self.Maid:Mark(self.Character.HumanoidRootPart:GetPropertyChangedSignal("Size"):Connect(function()
		self._characterMass = getModelMass(self.Character)
	end))
