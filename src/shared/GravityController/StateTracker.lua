...
function StateTrackerClass:Update(gravityUp, isGrounded, isInputMoving)
	...
	if not isGrounded and self.Controller.Character:GetAttribute("Grounded") then
		self.Controller.Character:SetAttribute("lastGroundedAt", tick())
	end 
	self.Controller.Character:SetAttribute("Grounded", not not isGrounded)
...
function StateTrackerClass:RequestJump()
	self.Jumped = true
	self.JumpTick = os.clock()
	self.Controller.Character:SetAttribute("LastJumpedAt", tick())
end

function StateTrackerClass:RequestDoubleJump()
